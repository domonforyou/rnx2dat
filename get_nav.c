...
/* test for nuaa rinex project --------------------------------------
	main();
*-------------------------------------------------------------------*/

int main(int argc, char **argv){
	if(argc!=4){
		printf("./a.out rinex_file toe mins \nsuch as : ./a.out 2015.p 439200 3 \n");
		return -1;
	}
	printf("start\n");
	int toe,mins,idn;
	...
	//printf("file is %s\n", argv[1]);
    /* read rinex file */
    stat=readrnxfp(fp,ts,te,tint,"",0,0,type,NULL,nav,NULL);

    idn = find_eph_by_toe(toe, idx, nav);
    //printf("find %d ephs \n",idn);
    for(i = 0; i < idn; i++){
	sprintf(filename, "%d.dat", nav->eph[idx[i]].sat);
	make_frame(nav, idx[i], mins, filename);
    }
    printf("gps week num is %d  %d\n", nav->eph->week, nav->eph[1].week);
    printf("gps max n is %d \n", nav->n);
    printf("gps a1 is %.12e\n",nav->eph->f1);
    printf("gps second 1 toc is %ld\n",nav->eph->toc.time);
    printf("gps second 1 toe is %lf\n",nav->eph->toes);
    printf("gps iode 384 is %d\n",nav->eph[419].iode);
    printf("gps iode 385 is %d\n",nav->eph[420].iode);
    fclose(fp);
    return stat;
}
